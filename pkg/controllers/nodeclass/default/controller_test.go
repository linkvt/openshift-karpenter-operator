	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			hcp := &hyperv1.HostedControlPlane{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						karpenterv1.KarpenterCoreE2EOverrideAnnotation: tc.annotation,
					},
				},
			...
			if tc.existingObject {
				...
				existingObject.(*karpenterv1.OpenshiftEC2NodeClass).Spec.Tags = map[string]string{"owned-by-user": "true"}
			}
...
func TestReconcileHCPCardinality(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			g := NewWithT(t)
			_, controller := newControllerWithHCPs(t, nil, testDefaultNodeClassProvider{}, tc.hcps...)
			_, err := controller.Reconcile(t.Context(), ctrl.Request{})
			if tc.wantError {
...
func TestNodeClassPredicate(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			object := &karpenterv1.OpenshiftEC2NodeClass{ObjectMeta: metav1.ObjectMeta{Name: tc.name}}
			g := NewWithT(t)
...
func (testDefaultNodeClassProvider) DefaultNodeClass(infraID string) (client.Object, controllerutil.MutateFn, error) {
	...
	mutate := func() error {
		...
		object.Spec = karpenterv1.OpenshiftEC2NodeClassSpec{
...
func defaultNodeClassObject() *karpenterv1.OpenshiftEC2NodeClass {
	return &karpenterv1.OpenshiftEC2NodeClass{ObjectMeta: metav1.ObjectMeta{Name: defaultNodeClassName}}
}
